...
if (!isset($_GET['delete_ApplianceID'], $_GET['Email'])) {
...
if (is_array($myArray)) {
    foreach ($myArray as $value) {
        if ($value == "Air conditioner" || $value == "Airconditioner") {
            $sql2 = "DELETE FROM AirConditioner WHERE AirHandlerId = '$ID' AND Email = '$email'";
        ...
        if ($value == "Heat pump" || $value == "HeatPump") {
